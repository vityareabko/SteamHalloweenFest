...
namespace razz
{
    public class InteractorTargetSpawner : EditorWindow
    {
            ...
            if (Physics.Raycast(ray, out hit, 1000f, playerMask))
            {
                if (PrefabUtility.GetPrefabAssetType(selectedPrefab) == PrefabAssetType.Regular)
                {
                    ...
                    if (addComponentsOnParent && CheckPrefabTypeSettings())
                    {
                        ...
                        if (addPivotOnPoint) _interactorObject.pivot = pivotGO.transform;
                    }
        ...
        public static InteractionTypeSettings ReturnSettingAsset(InteractorObject interactorObject)
        {
            ...
            else if (interaction >= 80 && interaction < 85)
            {
                if (interactorObject.pickableOneSettings != null)
                {
                    return interactorObject.pickableOneSettings;
                }
            }
            else if (interaction >= 85 && interaction < 90)
            {
                if (interactorObject.pickableTwoSettings != null)
                {
                    return interactorObject.pickableTwoSettings;
                }
            }
