...
namespace razz
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        public void Move(Vector3 move, bool crouch, bool jump, bool climb, bool use, bool changed, bool clicked)
        {
            ...
            if (_forwardAmount != 0) _interactionStates.playerMoving = true;
            else _interactionStates.playerMoving = false;

            if (move.magnitude == 0 && !crouch && !jump && !climb && !use && _interactionStates.playerGrounded && !clicked)
                _interactionStates.playerIdle = true;
            else _interactionStates.playerIdle = false;
            ...
            else _interactionStates.playerChanging = false;
        ...
        private void Climber()
        {
            ...
            else if (_playerRigidbody.position.y < _climbPos)
            {
                ...
                _playerRigidbody.linearVelocity = new Vector3(0, climbSpeed * _forwardAmount, 0);
        }
        ...
        void ControllerChange()
        {
            ...
            if (_interactionStates.playerOnVehicle) ExitVehicle();
            else EnterVehicle();
        }
